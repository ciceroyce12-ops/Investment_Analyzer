...
for ticker, meta in universe.items():
    try:
        ...
        if not news_items:
            news_items = [{
                "title": f"Market performance overview and trend analysis for {ticker}",
...
def validate_schema(data):
    ...
    required_keys = ["timestamp", "fx_rate_usd_idr", "top_opportunities", "full_universe", "historical_prices"]
    for key in required_keys:
        if key not in data:
            raise ValueError(f"Schema validation failed: Missing required key '{key}'")
    if not isinstance(data["full_universe"], list) or len(data["full_universe"]) == 0:
...
print("Quantitative engine data successfully generated and validated.")
