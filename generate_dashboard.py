...
TICKERS = [
    "SPY", "QQQ", "VT", "VGK", "EEM", "GLD", "BTC-USD",
    "TLT", "VNQ", "MSFT", "AAPL", "NVDA", "AMZN", "GOOGL"
]
...
print("-> Fetching live US 13-Week Treasury Bill yield for dynamic Risk-Free Rate...")
try:
    irx_hist = yf.Ticker("^IRX").history(period="5d")
    live_yield = irx_hist['Close'].iloc[-1]
    RISK_FREE_RATE = live_yield / 100 
    print(f"-> Live Risk-Free Rate dynamically set to: {live_yield:.2f}% ({RISK_FREE_RATE:.4f})\n")
except Exception as e:
    RISK_FREE_RATE = 0.045
    print(f"-> Could not fetch live macro data. Defaulting to historical average: {RISK_FREE_RATE*100}%\n")

# --- 3. DATA INGESTION & CALENDAR ALIGNMENT (TOTAL RETURN FIX) ---
print(f"-> Fetching full historical Total Return data (Adj Close) from {START_DATE} for {len(TICKERS)} assets...")
...
# --- 5. 4-YEAR FUTURE PREDICTION (MULTIVARIATE GBM) ---
forecast_results = {}
...
np.random.seed(42)
independent_Z = np.random.standard_normal((len(top_8_assets), steps, NUM_SIMULATIONS))
...
for idx, ticker in enumerate(top_8_assets):
    ...
    mu_geometric = ann_return[ticker]
    mu = mu_geometric + (0.5 * sigma**2)

    Z_ticker = correlated_Z[idx]
    step_returns = np.exp((mu - 0.5 * sigma**2) * dt + sigma * np.sqrt(dt) * Z_ticker)
...
fig_forecast.add_trace(go.Scatter(y=median_path, mode="lines", name="Median Expected Path", line=dict(color="cyan", width=3)))
fig_forecast.add_trace(go.Scatter(y=upper_bound, mode="lines", name="95th Percentile (Optimistic)", line=dict(color="orange", width=2, dash="dash")))
fig_forecast.add_trace(go.Scatter(y=lower_bound, mode="lines", name="5th Percentile (Pessimistic)", line=dict(color="magenta", width=2, dash="dash")))

fig_forecast.update_layout(
    title=f"<b>4-Year Future Price Forecast (Monte Carlo) for #1 Asset: {top_asset}</b>",
...
html_forecast = pio.to_html(fig_forecast, full_html=False, include_plotlyjs=False)
...
<body>
    <div class="container">
        ...
        <h2>2. 4-Year Monte Carlo Future Forecast ({top_asset})</h2>
        {html_forecast}
