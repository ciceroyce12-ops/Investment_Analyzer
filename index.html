...
fig_hist.update_layout(title="<b>Historical Backtest (Normalized)</b>", template="plotly_dark", hovermode="x unified", autosize=True)
...
fig_roll.update_layout(template="plotly_dark", yaxis_title="Annualized Volatility", showlegend=False, autosize=True)
...
fig_fcast.update_layout(title=f"<b>4-Year Forward Simulation ($10,000 Initial)</b>", xaxis_title="Trading Days", yaxis_title="Portfolio Value ($)", template="plotly_dark", autosize=True)
...
<head>
    ...
    <style>
        ...
        .container {{max-width:1200px;margin:auto;background:#111827;padding:30px;border-radius:12px;box-shadow:0 10px 25px rgba(0,0,0,0.5);}}
        h1 {{color:#00cc96;text-align:center;margin-bottom:5px;font-size:24px;}}
        .subtitle {{text-align:center;color:#9ca3af;margin-bottom:30px;font-size:14px;}}
        h2 {{color:#38bdf8;border-bottom:2px solid #374151;padding-bottom:8px;margin-top:40px;font-size:18px;}}
        .grid-2 {{display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px;}}
        .table-container {{overflow-x:auto;margin-top:15px;}}
        table.table-custom {{width:100%;border-collapse:collapse;background:#1f2937;border-radius:8px;overflow:hidden;font-size:14px;}}
...
print("✅ SUCCESS! index.html exported with responsive layout and fixed Google Maps links.")
